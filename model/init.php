...
class insert {
	...
	// 	$bind_success = $eks->bind_param("ssss", $this->nama, $this->email, $this->status, $this->password);
	// 	if (!$bind_success) {
	// 		die('Error binding parameters: ' . $eks->error);
	// 	}
		
	// 	$execute_success = $eks->execute();
	// 	if (!$execute_success) {
	// 		die('Error executing statement: ' . $eks->error);
	// 	}
		
	// 	$eks->close(); // Close the prepared statement
		
	// 	// Optionally, you can return true/false to indicate success or failure
